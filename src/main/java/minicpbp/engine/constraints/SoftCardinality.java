...
/**
 * Creates a soft cardinality constraint using the number of values whose number of occurrences is invalid as violation measure.
 ...
 */
public class SoftCardinality extends AbstractConstraint {

    private IntVar[] x;
    private int[] vals;
    private IntVar[] o;
    private IntVar nbViolations;
    private Solver cp;
    private IntVar[][] y;
    private int[][][] indicatorTable;

    public SoftCardinality(IntVar[] x, int[] vals, IntVar[] o, IntVar nbViolations, IntVar dummy) {
    ...
    public void post() {
        ...
        IntVar[] invalidOcc = new IntVar[vals.length];
        for (int j = 0; j < vals.length; j++) {
            IntVar[] vars = new IntVar[x.length];
            for (int i = 0; i < x.length; i++) {
                vars[i] = y[i][j];
            }
            invalidOcc[j] = isNotEqual(sum(vars), o[j]);
        }
	    cp.post(sum(invalidOcc, nbViolations));
        setActive(false);
    }
