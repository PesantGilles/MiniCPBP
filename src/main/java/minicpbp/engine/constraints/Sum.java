            ...
            if (nUnBounds.value() == 1) { // special case
                for (int j = 0; j < s; j++) {
                    ...
                    if (minState[0] + v == minState[1])
                        setLocalBelief(idx, v, beliefRep.one()); // no need to multiply by outside belief since this is the only non-zero belief
                    else
                ...
                for (int j = 0; j < s; j++) {
                    v = domainValues[j];
                    setLocalBelief(idx, v, beliefRep.multiply(op[0][minState[0] + v], outsideBelief(idx, v)));
                }
            }
